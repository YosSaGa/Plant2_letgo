  ...
  return (
    <AuthForm title="เข้าสู่ระบบ" subtitle="บันทึกสวนของคุณไว้กับบัญชี PlookPloen" onHome={() => onNavigate('/')}>
      <motion.form noValidate className="auth-form" onSubmit={submit} initial="hidden" animate="show" transition={{ staggerChildren: .07 }}>
        ...
        <motion.label variants={field}>รหัสผ่าน<span className="auth-input"><LockKeyhole size={18}/><input type={visible ? 'text' : 'password'} value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} placeholder="••••••••" /><button type="button" className="auth-eye" aria-label="แสดงหรือซ่อนรหัสผ่าน" onClick={() => setVisible(!visible)}>{visible ? <EyeOff size={18}/> : <Eye size={18}/>}</button></span></motion.label>
        <motion.a variants={field} className="auth-forgot" onClick={() => onNavigate('/forgot-password')}>ลืมรหัสผ่าน?</motion.a>
        ...
        <motion.button variants={field} type="button" className="auth-admin-link" onClick={() => onNavigate('/admin/login')}>🛠️ เข้าสู่ระบบผู้ดูแล</motion.button>
      </motion.form>
    </AuthForm>
  );
}
